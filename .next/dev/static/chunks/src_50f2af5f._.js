        ...
        "Home.useEffect": ()=>{
            const fetchPosts = {
                "Home.useEffect.fetchPosts": async ()=>{
                    ...
                    try {
                        // 从Supabase直接获取文章数据
                        const { data: postsData, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["supabase"].from('posts').select(`
            *, 
          ...
          `).eq('status', 'published').order('created_at', {
                        ...
                        const formattedPosts = (postsData || []).map({
                            "Home.useEffect.fetchPosts.formattedPosts": (post)=>({
                                    id: post.id,
                                    title: post.title,
                        ...
                        }["Home.useEffect.fetchPosts.formattedPosts"]);
                        // 按创建时间降序排序
                        formattedPosts.sort({
                            "Home.useEffect.fetchPosts": (a, b)=>new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
