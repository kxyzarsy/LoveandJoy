...
async function POST(request) {
    try {
        ...
        const connection = await createConnection();
        // 检查用户名ID是否已存在
        const [existingUserRows] = await connection.execute('SELECT id FROM user WHERE usernameId = ?', [
            usernameId
        ]);
        ...
        const [result] = await connection.execute('INSERT INTO user (name, email, password, usernameId, avatar, bio, role, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())', [
            name,
            email,
            password,
            usernameId,
        ...
        const [newUser] = await connection.execute('SELECT id, name, email, usernameId, avatar, backgroundImage, bio, role, inactive, createdAt, updatedAt FROM user WHERE id = ?', [
            result.insertId
        ]);
        await connection.end();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f3857596._.js.map